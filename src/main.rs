...
const TOKEN_META: &[(&str, alloy::primitives::Address, u8)] = &[
    ("WETH", alloy::primitives::Address::new(hex_literal::hex!("C02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2")), 18),
    ("USDC", alloy::primitives::Address::new(hex_literal::hex!("A0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48")), 6),
    ("USDT", alloy::primitives::Address::new(hex_literal::hex!("dAC17F958D2ee523a2206206994597C13D831ec7")), 6),
    ("DAI",  alloy::primitives::Address::new(hex_literal::hex!("6B175474E89094C44Da98b954EedeAC495271d0F")), 18),
    ("WBTC", alloy::primitives::Address::new(hex_literal::hex!("2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599")), 8),
...
fn format_amount(amount: &alloy::primitives::I256, token: alloy::primitives::Address) -> String {
    ...
    let dec = TOKEN_META.iter().find(|m| m.1 == token).map(|m| m.2).unwrap_or(18);
    let sym = TOKEN_META.iter().find(|m| m.1 == token).map(|m| m.0).unwrap_or("???");
    let prefix = if sign.is_negative() { "-" } else { "" };
    let raw: u128 = abs.to::<u128>();
    let div = 10u128.pow(dec as u32);
    let int_part = raw / div;
    let frac = raw % div;
    format!("{}{}.{:0>width$} {}", prefix, int_part, frac, sym, width = dec as usize)
}
...
pub const DEFAULT_BLACKLIST: &[alloy::primitives::Address] = &[
    alloy::primitives::Address::new(hex_literal::hex!("C02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2")),
    alloy::primitives::Address::new(hex_literal::hex!("7a250d5630B4cF539739dF2C5dAcb4c659F2488D")),
    alloy::primitives::Address::new(hex_literal::hex!("E592427A0AEce92De3Edee1F18E0157C05861564")),
    alloy::primitives::Address::new(hex_literal::hex!("68b3465833fb72A70ecDF485E0e4C7bD8665Fc45")),
    alloy::primitives::Address::new(hex_literal::hex!("000000000004444c5dc75Cb358380D2e08dE62B0")),
    alloy::primitives::Address::new(hex_literal::hex!("BA12222222228d8Ba445958a75a0704d566BF2C8")),
    alloy::primitives::Address::new(hex_literal::hex!("1111111254EEB25477B68fb85Ed929f73A960582")),
    alloy::primitives::Address::new(hex_literal::hex!("111111125421cA6dc452d289314280a0f8842A65")),
    alloy::primitives::Address::new(hex_literal::hex!("C0FFEE0000000000000000000000000000000000")),
];
...
pub const DEFAULT_TOKENS: &[alloy::primitives::Address] = &[
    ...
    alloy::primitives::Address::new(hex_literal::hex!("C02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2")),
    alloy::primitives::Address::new(hex_literal::hex!("dAC17F958D2ee523a2206206994597C13D831ec7")),
    alloy::primitives::Address::new(hex_literal::hex!("A0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48")),
    alloy::primitives::Address::new(hex_literal::hex!("6B175474E89094C44Da98b954EedeAC495271d0F")),
    alloy::primitives::Address::new(hex_literal::hex!("2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599")),
];
        ...
        .map(|block_number| {
            async move {
                ...
                let bundles = detect_sandwiches(
                    block_number, &data.flows, &data.raw_logs,
                    data.coinbase, DEFAULT_BLACKLIST, DEFAULT_TOKENS,
                );
...
async fn run_scan(cfg: config::ScanConfig) -> anyhow::Result<()> {
    ...
    let from = if cfg.resume {
        db::last_scanned_block(&pool).await?.map(|b| b + 1).unwrap_or(cfg.from)
    } else {
        cfg.from
    };

    info!("scanning blocks {}..{} (concurrency={})", from, cfg.to, cfg.concurrency);
        ...
        for result in results {
            match result {
                Ok((block_num, bundles)) => {
                    ...
                    if !bundles.is_empty() { db::insert_sandwiches(&pool, &bundles).await?; }
...
async fn run_replay(cfg: config::ReplayConfig) -> anyhow::Result<()> {
    ...
    info!("replay queued: scanner will re-scan from block {} on its next iteration", cfg.from_block);
    Ok(())
}

async fn run_peek(cfg: config::PeekConfig) -> anyhow::Result<()> {
    let provider = rpc::RpcClient::new(&cfg.rpc_url)?;
    eprintln!("peeking blocks {}..{} (concurrency={})", cfg.from, cfg.to, cfg.concurrency);
                    ...
                    if !bundles.is_empty() {
                        ...
                        for bundle in &bundles {
                            match cfg.format.as_str() {
                                ...
                                "summary" => {
                                    let profit_str: Vec<String> = bundle.profit.iter()
                                        .map(|p| format_amount(&p.amount, p.token))
                                        .collect();
                                    let weth = alloy::primitives::Address::new(hex_literal::hex!("C02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"));
                                    let weth_profit: i128 = bundle.profit.iter()
                                        .filter(|p| p.token == weth)
                                        .map(|p| p.amount.into_sign_and_abs().1.to::<u128>() as i128)
                                        .sum();
                                    ...
                                    // Cost breakdown
                                    let direct_eth = bundle.expense_wei.saturating_sub(bundle.gas_cost_wei);
    ...
    eprintln!("peek complete. {} sandwiches in {} blocks out of {} scanned",
        total, hits, cfg.to - cfg.from + 1);
    Ok(())
}
...
async fn run_export(cfg: config::ExportConfig) -> anyhow::Result<()> {
    ...
    for row in rows {
        ...
        if cfg.format == "json" {
            ...
            println!("{},{:?},{},{}", block, hex::encode(attacker), front_tx, back_tx);
        }
    }
...
async fn run_serve(config_path: &str) -> anyhow::Result<()> {
    ...
    info!("serve: starting from block {} (delay={})", cfg.from_block, cfg.delay_blocks);
    ...
    loop {
        ...
        let state = match db::read_scan_state(&pool).await {
            Ok(s) => s,
            Err(e) => { tracing::error!("read_scan_state: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue; }
        };
        ...
        if state.pending_replay_from != 0 {
            ...
            if let Err(e) = db::delete_sandwiches_from(&pool, from).await {
                tracing::error!("delete_sandwiches_from: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue;
            }
            if let Err(e) = db::delete_blocks_scanned_from(&pool, from).await {
                tracing::error!("delete_blocks_scanned_from: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue;
            }
            if let Err(e) = db::reset_scan_state_to(&pool, from).await {
                tracing::error!("reset_scan_state_to: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue;
            }
            if let Err(e) = db::clear_pending_replay_from(&pool).await {
                tracing::error!("clear_pending_replay_from: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue;
            }
            if !state.enabled {
                info!("auto-resuming scanner due to pending replay");
                if let Err(e) = db::set_scan_enabled(&pool, true).await {
                    tracing::error!("set_scan_enabled: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue;
                }
            }
            continue;
        }
        ...
        let chain_head = match provider.block_number().await {
            Ok(h) => h,
            Err(e) => { tracing::error!("block_number: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue; }
        };
        {
            ...
            if chain_head != LAST_HEAD.load(std::sync::atomic::Ordering::Relaxed) {
                if let Err(e) = db::update_chain_head(&pool, chain_head).await {
                    tracing::error!("update_chain_head: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue;
                }
        ...
        let data = match rpc::fetch_block(&provider, block_to_scan).await {
            Ok(d) => d,
            Err(e) => { tracing::error!("fetch_block {}: {:?}", block_to_scan, e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue; }
        };
        let bundles = detector::sandwich::detect_sandwiches(
            block_to_scan, &data.flows, &data.raw_logs,
            data.coinbase, DEFAULT_BLACKLIST, DEFAULT_TOKENS,
        );
        ...
        let state = match db::read_scan_state(&pool).await {
            Ok(s) => s,
            Err(e) => { tracing::error!("read_scan_state (post-fetch): {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue; }
        };
        ...
        if !bundles.is_empty() {
            if let Err(e) = db::insert_sandwiches(&pool, &bundles).await {
                tracing::error!("insert_sandwiches: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue;
            }
        }
        if let Err(e) = db::mark_block_scanned(&pool, block_to_scan, count).await {
            tracing::error!("mark_block_scanned: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue;
        }
        if let Err(e) = db::advance_scan_state(&pool, block_to_scan + 1).await {
            tracing::error!("advance_scan_state: {:?}", e); tokio::time::sleep(std::time::Duration::from_secs(2)).await; continue;
        }
